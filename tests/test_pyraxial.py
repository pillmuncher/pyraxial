...
def test_enclose():
    ...
    a = Rect((1, 2, 3, 4))
    assert Rect.enclose() == Rect.EMPTY
    assert Rect.enclose(Rect.EMPTY) == Rect.EMPTY
    assert Rect.enclose(Rect.EMPTY, Rect.EMPTY) == Rect.EMPTY
    assert Rect.enclose(a) == a
    assert Rect.enclose(a, a) == a
    assert Rect.enclose(a, Rect.EMPTY) == a
    assert Rect.enclose(Rect.EMPTY, a) == a
    rects = (
        Rect.EMPTY,
        Rect((1, 2, 3, 4)),
        Rect((2, 3, 4, 5)),
    ...
    for a, b, c, d in permutations(rects):
        assert Rect.enclose(a, b, c, d) == expected
...
def test_overlap():
    ...
    c = Rect((2, 3, 3, 4))
    d = Rect((5, 6, 7, 8))
    assert Rect.overlap() == Rect.PLANE
    assert Rect.overlap(Rect.EMPTY) == Rect.EMPTY
    assert Rect.overlap(Rect.EMPTY, Rect.EMPTY) == Rect.EMPTY
    assert Rect.overlap(a) == a
    assert Rect.overlap(a, a) == a
    assert Rect.overlap(a, Rect.EMPTY) == Rect.EMPTY
    assert Rect.overlap(Rect.EMPTY, a) == Rect.EMPTY
    assert Rect.overlap(a, b) == c
    assert Rect.overlap(b, a) == c
    assert Rect.overlap(a, d) == Rect.EMPTY
    assert Rect.overlap(d, a) == Rect.EMPTY
